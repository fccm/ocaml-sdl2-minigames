...
let rect_intersects_with_line rect (p1, p2) =
  match Rect.intersect_rect_and_line ~rect ~p1 ~p2 with
  | Some _ -> true
  | None -> false
...
let step_towers towers req chuncks bullets foes t =
  let towers =
    match req with
    | None -> towers
    | Some t_pos ->
        let req_rect =
          let x, y = t_pos in
          Rect.make4 (x-10) (y-10) 20 20
        in
        if List.exists (fun ps -> rect_intersects_with_line req_rect ps) chuncks
        then towers
        else
          let tower = {
            t_pos;
...
let () =
  ...
  let new_wave foes t =
    ...
    let new_foes =
      let s = 300 + Random.int 400 in
      let duration = 6000 + Random.int 6000 in
      List.init n (fun i ->
        let t_start = t + i * s in
        let f_timeline =
          timeline_of_chuncks chuncks_and_len t_start duration
        in
        { t_start; duration; f_timeline; f_pos = (0, 0); }
      )
    in
    (new_foes @ foes)
  in
  let new_wave_opt foes t =
    if Random.int 1000 < 8
    then new_wave foes t
    else foes
  in
  let towers = [] in
  let bullets = [] in
  ...
  let rec main_loop foes towers bullets =
    ...
    let foes = new_wave_opt foes t in
    let foes = step_foes foes t in
    let towers, bullets = step_towers towers req chuncks bullets foes t in
