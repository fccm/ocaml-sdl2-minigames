...
let find_target tower foes =
  ...
  if List.length foes = 0 then None else
    let dists =
      List.map (fun pos ->
        let len = distance tower.t_pos pos in
        (len, pos)
      ) foes
    in
    let _, foe =
...
let step_towers towers bullets foes t =
  ...
  | tower :: towers ->
      ...
      then aux acc1 (tower :: acc2) towers
        ...
        | Some foe ->
            ...
            let bullet =
              let len = int_of_float (distance tower.t_pos foe) in
              let d = len * 3 in
              let ps = (tower.t_pos, foe) in
